 ...
 * events.
 */
public class ClosableWindow {

  /**
   * Fired just before the browser window closes or navigates to a different
   * site.
   */
  public static class ClosingEvent extends GwtEvent<ClosableWindow.ClosingHandler> {
    /**
     * The event type.
     */
    ...
    @Override
    protected void dispatch(final ClosingHandler handler) {
      handler.onWindowClosing(this);
    }
  }

  /**
   * Handler for {@link ClosableWindow.ClosingEvent} events.
   */
  public interface ClosingHandler extends EventHandler {
     ...
     *          the event
     */
    void onWindowClosing(ClosableWindow.ClosingEvent event);
  }
  ...
  public static class Location {
    ...
    public static UrlBuilder createUrlBuilder() {
      ...
      if (hash != null && hash.length() > 0) {
        // Decode the hash now, because UrlBuilder.buildString() later encodes
        // it.
      ...
      final Map<String, List<String>> params = getParameterMap();
    ...
    public static String getParameter(final String name) {
      ...
      final List<String> paramsForName = listParamMap.get(name);
    ...
    /**
     * Returns an immutable Map of the URL query parameters for the host page at
     * the time this method was called. Any changes to the window's location
     * will be reflected in the result of subsequent calls.
     *
     ...
     */
    static Map<String, List<String>> buildListParamMap(final String queryString) {
      ...
      for (final Map.Entry<String, List<String>> entry : out.entrySet()) {
   ...
   */
  public static class ScrollEvent extends GwtEvent<ClosableWindow.ScrollHandler> {
    /**
     * The event type.
     */
    static final Type<ClosableWindow.ScrollHandler> TYPE = new Type<ClosableWindow.ScrollHandler>();
    ...
    /**
     * Construct a new {@link ClosableWindow.ScrollEvent}.
     *
     * @param scrollLeft
     ...
     */
    private ScrollEvent(final int scrollLeft, final int scrollTop) {
      this.scrollLeft = scrollLeft;
      this.scrollTop = scrollTop;
    }

    static Type<ClosableWindow.ScrollHandler> getType() {
      return TYPE;
    }
  ...
  /**
   * Handler for {@link ClosableWindow.ScrollEvent} events.
   */
  public interface ScrollHandler extends EventHandler {
     ...
     *          the event
     */
    void onWindowScroll(ClosableWindow.ScrollEvent event);
  }

  private static class WindowHandlers extends HandlerManager implements HasCloseHandlers<ClosableWindow>, HasResizeHandlers {
    ...
    @Override
    public HandlerRegistration addCloseHandler(final CloseHandler<ClosableWindow> handler) {
   ...
   */
  public static HandlerRegistration addCloseHandler(final CloseHandler<ClosableWindow> handler) {
   ...
   * @return returns the handler registration
   */
  public static HandlerRegistration addResizeHandler(final ResizeHandler handler) {
    maybeInitializeCloseHandlers();
   ...
   * @deprecated use
   *             {@link ClosableWindow#addWindowClosingHandler(ClosingHandler)}
   *             and {@link ClosableWindow#addCloseHandler(CloseHandler)}
   *             instead
   */
  @Deprecated
  public static void addWindowCloseListener(final WindowCloseListener listener) {
    BaseListenerWrapper.WrapWindowClose.add(listener);
  }

  /**
   * Adds a {@link ClosableWindow.ClosingEvent} handler.
   *
   * @param handler
   *          the handler
   * @return returns the handler registration
  ...
  public static HandlerRegistration addWindowClosingHandler(final ClosingHandler handler) {
    maybeInitializeCloseHandlers();
    return addHandler(ClosableWindow.ClosingEvent.getType(), handler);
  }
   ...
   * @deprecated use {@link ClosableWindow#addResizeHandler(ResizeHandler)}
   *             instead
   */
  @Deprecated
  public static void addWindowResizeListener(final WindowResizeListener listener) {
    BaseListenerWrapper.WrapWindowResize.add(listener);
  }

  /**
   * Adds a {@link ClosableWindow.ScrollEvent} handler.
   *
   * @param handler
   *          the handler
   * @return returns the handler registration
   */
  public static HandlerRegistration addWindowScrollHandler(final ClosableWindow.ScrollHandler handler) {
    maybeInitializeCloseHandlers();
    maybeInitializeScrollHandlers();
    return addHandler(ClosableWindow.ScrollEvent.getType(), handler);
  }
   ...
   * @deprecated use
   *             {@link ClosableWindow#addWindowScrollHandler(ScrollHandler)}
   *             instead
   */
  @Deprecated
  public static void addWindowScrollListener(final WindowScrollListener listener) {
   ...
   */
  public static void enableScrolling(final boolean enable) {
   ...
   *
   * @param dx
   *          A positive or a negative number that specifies how many pixels to
   *          move the left edge by
   * @param dy
   *          A positive or a negative number that specifies how many pixels to
   *          move the top edge by
   */
   ...
   */
  public static native JavaScriptObject openWithReference(String url, String name, String features) /*-{
   ...
   */
  @Deprecated
  public static void removeWindowCloseListener(final WindowCloseListener listener) {
   ...
   */
  @Deprecated
  public static void removeWindowResizeListener(final WindowResizeListener listener) {
   ...
   */
  @Deprecated
  public static void removeWindowScrollListener(final WindowScrollListener listener) {
  ...
  static String onClosing() {
    if (closeHandlersInitialized) {
      final ClosableWindow.ClosingEvent event = new ClosableWindow.ClosingEvent();
      fireEvent(event);
  ...
  static void onScroll() {
    if (scrollHandlersInitialized) {
      fireEvent(new ClosableWindow.ScrollEvent(getScrollLeft(), getScrollTop()));
    }
  }
   ...
   */
  private static <H extends EventHandler> HandlerRegistration addHandler(final GwtEvent.Type<H> type, final H handler) {
   ...
   *          the event
   */
  private static void fireEvent(final GwtEvent<?> event) {
    if (handlers != null) {
